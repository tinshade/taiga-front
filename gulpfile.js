...
const availableThemes = JSON.stringify(
...
paths.libs = [
    ...
    paths.modules +
        "angular-translate-interpolation-messageformat/angular-translate-interpolation-messageformat.js",
    ...
    paths.app + "js/msal-sso.js", //* Add this to include MSAL in your project
];
...
gulp.task(
    "clear",
    gulp.series("clear-sass-cache", function (done) {
        cache.clearAll();
        done();
    })
);
...
gulp.task("copy-index", function () {
    return gulp.src(paths.tmp + "index.html").pipe(gulp.dest(paths.dist));
});
...
gulp.task(
    "jade-watch",
    gulp.series("jade-inheritance", "copy-index", "template-cache")
);
...
gulp.task(
    "compile-themes",
    gulp.series(new Array(themes.size).fill("compile-theme"))
);
...
gulp.task("emoji", function (cb) {
    ...
    emojis.forEach(function (emoji) {
        Jimp.read(
            emojiPath + "img-twitter-64/" + emoji.image,
            function (err, lenna) {
                if (err) throw err;
...
gulp.task("conf", function () {
    return gulp.src(["conf/conf.example.json"]).pipe(gulp.dest(paths.dist));
});

gulp.task("app-loader", function () {
    return gulp
        .src("app-loader/app-loader.coffee")
...
gulp.task("locales", function () {
    var plugins = gulp.src(paths.app + "modules/**/locales/*.json").pipe(
        rename(function (localeFile) {
            ...
            gulpif(
                ...
                cache(coffeelint(), {
                    key: function (lintFile) {
                        return (
                            "coffee-lint" + lintFile.contents.toString("utf8")
                        );
                    },
...
gulp.task("moment-locales", function () {
    replace_lang_path = { "zh-cn": "zh-hans", "zh-tw": "zh-hant" };

    return gulp
        .src(paths.modules + "moment/locale/*")
        .pipe(gulpif(isDeploy, uglify()))
        .pipe(
...
gulp.task(
    "app-watch",
    gulp.series("coffee", "conf", "locales", "moment-locales", "app-loader")
);
...
gulp.task(
    ...
    gulp.series(
        ...
        function () {
            return gulp
                .src(paths.distVersion + "js/app.js")
                .pipe(sourcemaps.init())
                .pipe(uglify())
                .pipe(sourcemaps.write("./maps"))
                .pipe(gulp.dest(paths.distVersion + "js/"));
...
gulp.task("copy-images", function () {
    return gulp
        .src([
            paths.app + "/images/**/*",
            paths.app + "/modules/compile-modules/**/images/*",
        ])
...
gulp.task("copy-ckeditor-translations", function () {
    return gulp
        .src(
            paths.modules +
                "taiga-html-editor/packages/ckeditor5-build-classic/build/translations/*"
        )
...
gulp.task("copy-hljs-languages", function () {
    return gulp
        .src(paths.modules + "@highlightjs/cdn-assets/languages/*")
...
gulp.task("express", function (cb) {
    ...
    app.use(
        "/" + version + "/js",
        express.static(__dirname + "/dist/" + version + "/js")
    );
    app.use(
        "/" + version + "/styles",
        express.static(__dirname + "/dist/" + version + "/styles")
    );
    app.use(
        "/" + version + "/images",
        express.static(__dirname + "/dist/" + version + "/images")
    );
    app.use(
        "/" + version + "/emojis",
        express.static(__dirname + "/dist/" + version + "/emojis")
    );
    app.use(
        "/" + version + "/svg",
        express.static(__dirname + "/dist/" + version + "/svg")
    );
    app.use(
        "/" + version + "/partials",
        express.static(__dirname + "/dist/" + version + "/partials")
    );
    app.use(
        "/" + version + "/fonts",
        express.static(__dirname + "/dist/" + version + "/fonts")
    );
    app.use(
        "/" + version + "/locales",
        express.static(__dirname + "/dist/" + version + "/locales")
    );
    app.use(
        "/" + version + "/maps",
        express.static(__dirname + "/dist/" + version + "/maps")
    );
    app.use(
        "/" + version + "/ckeditor-translations",
        express.static(
            __dirname + "/dist/" + version + "/ckeditor-translations"
        )
    );
    app.use(
        "/" + version + "/highlightjs-languages",
        express.static(
            __dirname + "/dist/" + version + "/highlightjs-languages"
        )
    );
...
gulp.task("watch", function (cb) {
    ...
    gulp.watch(
        paths.styles_dependencies,
        gulp.parallel(["styles-dependencies"])
    );
    gulp.watch(paths.svg, gulp.parallel(["copy-svg"]));
    ...
    gulp.watch(
        [paths.locales, paths.modulesLocales],
        gulp.parallel(["locales"])
    );
...
gulp.task(
    ...
    gulp.series("default", function () {
        return gulp
            .src([paths.distVersion + "js/app.js", paths.tmp + "**/*.html"])
            .pipe(
                utils.unusedCss({
